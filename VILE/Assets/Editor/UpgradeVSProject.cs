...
class UpgradeVSProject {
	[MenuItem("Tools/Target Framework/Reset Solution Frameworks to v4.7")]
	static void UpgradeSolutions() {
		...
		if(csprojFile != null) {
			for(int i = 0; i < csprojFile.Length; i++) {
				if(ReplaceInFile(csprojFile[i], "ToolsVersion=\"4.0\"", "ToolsVersion=\"4.7\"")) {
					toolsUpdates.Add(Path.GetFileNameWithoutExtension(csprojFile[i]));
				}

				if(ReplaceInFile(csprojFile[i], "<TargetFrameworkVersion>v3.5</TargetFrameworkVersion>", "<TargetFrameworkVersion>v4.7</TargetFrameworkVersion>")) {
		...
		if(unityProjFile != null) {
			for(int i = 0; i < unityProjFile.Length; i++) {
				if(ReplaceInFile(unityProjFile[i], "ToolsVersion=\"4.0\"", "ToolsVersion=\"4.7\"")) {
					toolsUpdates.Add(Path.GetFileNameWithoutExtension(unityProjFile[i]));
				}

				if(ReplaceInFile(unityProjFile[i], "<TargetFrameworkVersion>v3.5</TargetFrameworkVersion>", "<TargetFrameworkVersion>v4.7</TargetFrameworkVersion>")) {
